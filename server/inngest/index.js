...
const autoCheckOut = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Wait 9 hours before sending reminder
    await step.sleep("wait-for-9-hours", "9h");
    ...
    if (!attendance?.checkOut) {
      ...
      // Wait 1 more hour then auto-checkout if still not checked out
      await step.sleep("wait-for-1-hour", "1h");
...
const leaveApplicationReminder = inngest.createFunction(
  { id: "leave-application-reminder", triggers: [{ event: "leave/pending" }] },
  async ({ event, step }) => {
    // Fixed: was LeaveApplicationId (capital L) — must match what createLeave sends
    ...
    if (leaveApplication?.status === "PENDING") {
      ...
      await sendEmail({
        ...
        body: `
          <div style="max-width: 600px;">
            <h2>Hi Admin, 👋</h2>
            <p style="font-size: 16px;">A leave application from ${employee.department} is still pending:</p>
            <p style="font-size: 18px; font-weight: bold; color: #007bff; margin: 8px 0;">
...
//         // Fixed: sendEmail was not awaited — errors were silently swallowed
  ...
  async ({ step }) => {
    ...
    const today = await step.run("get-today-date", () => {
      const istDate = new Intl.DateTimeFormat("en-CA", {
        ...
        month:    "2-digit",
        day:      "2-digit",
      }).format(new Date()); // always "YYYY-MM-DD" regardless of server locale
