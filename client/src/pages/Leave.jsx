...
const Leave = () => {
  ...
  const annualCount = approvedLeaves.filter((l)=> l.type === "LOSS_OF_PAY").length;

  const leaveStats =[
    {label:"Sick Leave", value:sickCount, icon:ThermometerIcon},
    {label:"Casual Leave", value:casualCount, icon:UmbrellaIcon},
    {label:"Loss of Pay ", value:annualCount, icon:PalmtreeIcon},
  ]
            ...
            {leaveStats.map((s)=>(
              <div key={s.label} className="card card-hover p-5 sm:p-6 flex items-center gap-4 relative overflow-hidden group ">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-slate-900 tracking">{s.value} <span className="text-sm font-normal text-slate-400">taken</span></p>
                </div>
              </div>
            ))}
          </div>
