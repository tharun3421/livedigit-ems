...
const MyProfile = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                // Uses the existing profile endpoint — returns employee data for logged-in user
    ...
    return (
        <div className="animate-fade-in max-w-3xl mx-auto space-y-5 pb-10">
            ...
            {/* ── Work Schedule ── */}
            {(ws.shiftStart || ws.weekOff?.length) && (
            ...
            {/* ── Leave Summary ── */}
            <Section title="Approved Leaves (This Year)">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            <Section title="Salary Details">
                <Row icon={BadgeIndianRupeeIcon} label="Basic Salary"
                    value={`₹${profile.basicSalary?.toLocaleString("en-IN") ?? 0}`} />
                <Row icon={BadgeIndianRupeeIcon} label="Allowances"
                    value={`₹${profile.allowances?.toLocaleString("en-IN") ?? 0}`} />
                <Row icon={BadgeIndianRupeeIcon} label="Deductions"
                    value={`₹${profile.deductions?.toLocaleString("en-IN") ?? 0}`} />
