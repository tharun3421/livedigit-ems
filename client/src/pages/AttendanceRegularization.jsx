...
import {
  ...
  Loader2Icon, SearchIcon, FilterIcon,
} from "lucide-react"
...
const AttendanceRegularization = () => {
  ...
  const handleAction = async (id, status) => {
    setReviewing(id)
    try {
      await api.patch(`/regularization/${id}`, {
        status,
        adminRemark: adminRemarks[id] || "",
      })
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-slate-400 text-sm">No {filter !== "ALL" ? filter.toLowerCase() + " " : ""}requests found</p>
          ...
          <div className="divide-y divide-slate-800/60">
            {filtered.map((req) => {
              ...
              return (
                <div key={req._id} className="px-4 sm:px-6 py-4 sm:py-5 hover:bg-slate-800/20 transition-colors">
                  {/* Employee info */}
