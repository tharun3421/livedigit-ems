...
const StatCard = ({ icon: Icon, value, title, subtitle, hint, onClick }) => (
  <div onClick={onClick}
    ...
    <div>
      ...
      {hint && <p className="text-[10px] text-indigo-400 mt-1 font-medium opacity-0 group-hover:opacity-100 transition-opacity">{hint}</p>}
    </div>
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-5">
        <StatCard icon={CalendarIcon} value={data.currentMonthAttendance}
          title="Days Present" subtitle="This month" hint="Click to view calendar →"
          onClick={() => setShowCalendar(true)} />
        <StatCard icon={FileTextIcon} value={data.pendingLeaves}
          title="Pending Leave Requests" subtitle="Awaiting approval" hint="Click to view leaves →"
          onClick={() => navigate("/leave")} />
        <StatCard icon={IndianRupeeIcon}
          value={data.latestPayslip ? `₹ ${data.latestPayslip.netSalary?.toLocaleString()}` : "N/A"}
          title="Latest Payslip" subtitle="Most recent payout"
      ...
      <div className="mb-5 flex items-start gap-3 p-4 rounded-xl bg-amber-500/8 border border-amber-500/20">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Every <strong className="text-amber-300">3 late check-ins</strong> in a month results in <strong className="text-amber-300">1 day salary deduction</strong>. Please ensure timely check-in to avoid deductions.
          </p>
        </div>
      </div>
