...
//                     <input type="date" name="date" required defaultValue={initialData?.joinData ? new Date(intialData.joinData).toISOString().split("T")[0]:""} />
//                 </div>
...
//             </div>
//             <div>
//                 <label className="block mb-2">Deducations</label>
//                 <input type="number" name="deducations"  required min="0" step="0.01" defaultValue={initialData?.deducations || 0}/>
//             </div>
//             {isEditMode && (
//                 <div>
...
//         <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-2" onClick={()=> (onCancel ? onCancel() : navigate(-1))}>
//             <button className="btn-secondary" type="button">Cancle</button>
...
const EmployeeForm = ({ initialData, onSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const method = isEditMode ? "put" : "post";
      await api[method](url, body);                        // ← send JSON not FormData
